<script lang="ts">
	...
	function validateForm(): boolean {
		...
		} else if (objective.length > 1000) {
			errors.objective = 'Objective must be less than 1000 characters';
		}
		...
		if (enableSchedule && !cronExpression.trim()) {
			errors.cronExpression = 'Cron expression is required when schedule is enabled';
		}
...
<div class="container mx-auto py-6 max-w-3xl space-y-6">
	...
	<div>
		...
		<p class="text-muted-foreground mt-1">Define a new goal for your async agent</p>
	</div>
	...
	<form on:submit|preventDefault={handleSubmit} class="space-y-6">
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Define what you want the agent to accomplish</Card.Description>
			</Card.Header>
				...
				<div class="space-y-2">
					...
					{:else}
						<p class="text-sm text-muted-foreground">
							{objective.length} / 1000 characters (minimum 10)
						</p>
					{/if}
				</div>
		...
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Configure automatic execution schedule</Card.Description>
			</Card.Header>
			<Card.Content class="space-y-4">
				<div class="flex items-center gap-2">
					<input
