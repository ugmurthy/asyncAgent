<script lang="ts">
  ...
  async function renderChart() {
    ...
    try {
      ...
      if (!chart || chart.trim() === '') {
        container.innerHTML = '<div class="text-gray-400 text-sm p-4">No graph data available</div>';
        return;
      }
...
<div class="mermaid-wrapper w-full overflow-x-auto flex justify-center p-4 bg-white rounded-lg border border-gray-200" bind:this={container}>
